...
class SortedType{
	public:
		...
		void MakeEmtpy();
		  ...
		  //useful for debuging & showing cpactiy. Also better than get length bc it shows the capcity instead of the length.
	...
	private:
		  ...
		  int currentPos;
		  int capacity;
		  //int MAX_ITEMS; dont need anymore
...
SortedType::SortedType(const SortedType& rhs){
	...
	delete[] info;
	info = new ItemType[capacity];//litteraly replace all MAX_Items with capacity
...
void SortedType::resize() {
	capacity *= 2;
	ItemType* newInfo = new ItemType[capacity];
...
void SortedType::operator++(){   //pre-increment: increases the size of the info array by 1.
	ItemType* infoCopy = new ItemType[capacity + 1];
...
void SortedType::operator++(int){   //post-increment: increases the size of the info array by 1.
	ItemType* infoCopy = new ItemType[capacity + 1];
	for(int i = 0; i < capacity; i++){
		infoCopy[i] = info[i];
	}
...
int SortedType::GetCapacity() const
{
	return capacity;
}
